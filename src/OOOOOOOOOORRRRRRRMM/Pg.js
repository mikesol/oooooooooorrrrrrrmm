...
export const parsePostgresUrlImpl = (left) => (right) => (url) => {
  const regex =
    /^postgresql:\/\/(?<user>[^@]+)@(?<host>[^:]+):(?<port>\d+)\/(?<database>.+)$/;
...
export const newClientImpl =
  (left) => (right) => (host) => (port) => (user) => (database) => async () => {
...
export const newClientConnectionStringImpl =
  (left) => (right) => (connectionString) => async () => {
    const client = new Client({
      connectionString,
    });
...
export const runSqlCommandImpl =
  (left) => (right) => (client) => (sql) => (values) => async () => {
    try {
